...
def load_baseline_uids():
    ...
    baseline_path = Path(__file__).parent / "data" / "baseline" / "baseline_annotated.parquet"
    ...
    if baseline_path.exists():
        try:
            ...
            st.warning(f"⚠️ Nie udało się wczytać baseline: {e}")
            return set()
    else:
        st.warning(f"⚠️ Nie znaleziono pliku baseline: {baseline_path}")
        return set()
