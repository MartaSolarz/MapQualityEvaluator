...
def generate_html(df_samples, output_path, review_label='NO', change_to='YES'):
    """
    ...
    Args:
        ...
        review_label: Etykieta próbek które przeglądamy (NO, YES, INVALID)
...
<head>
    ...
    <title>Review {review_label} Samples - Change to {change_to}</title>
    <style>
        * {{
...
<body>
    ...
    <h1>🔍 Review {review_label} Samples → Change to {change_to}</h1>
    
    <div class="instructions">
        <strong>Instrukcja:</strong> Przeglądasz próbki z etykietą <strong>{review_label}</strong>. 
        Kliknij na <strong>tło karty</strong> żeby zaznaczyć do zmiany na <strong>{change_to}</strong>. 
        Kliknij na <strong>obrazek</strong> żeby powiększyć. W modalu: SPACE/ENTER = toggle, ESC = zamknij.
    </div>
    
    <div class="stats">
        <strong>Total {review_label}:</strong> <span id="total">{len(df_samples)}</span> | 
...
"""
    ...
    for _, row in df_samples.iterrows():
        ...
        score = row.get('score', '')
        score_text = f'<div class="score">Score: {score:.3f}</div>' if score != '' else ''
...
"""
    ...
    <script>
        ...
        const REVIEW_LABEL = '{review_label}';
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Przykłady użycia:
  # Przegląd NO → zmiana na YES (domyślnie):
  python3 generate_review_html.py --file data/train.parquet
  ...
  python3 generate_review_html.py --file annotations.jsonl --parquet-source data/full.parquet
    ...
    parser.add_argument('--review-label', type=str, default='NO',
                        choices=['NO', 'YES', 'INVALID'],
                        help='Która etykieta do przeglądu (default: NO)')
