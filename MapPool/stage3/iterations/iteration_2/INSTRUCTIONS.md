# Instrukcje adnotacji - Iteracja 2

## 📋 Zadanie
Zaadnotuj 400 **VALID** próbek jako:
...
```bash
cd MapPool/stage3
streamlit run annotation_interface.py -- --iteration 2 --target 400
```
...
- `iteration_2/annotated.parquet` (gdy osiągniesz target)
- `iteration_2/annotations_progress.jsonl` (progress)
...
Interface wybierze więcej próbek niż 400 (buffer na invalid URLs).
Oczekiwany % invalid: ~38%, więc wybranych zostanie ~645 próbek.
Ale adnotujesz tylko do osiągnięcia 400 VALID!
